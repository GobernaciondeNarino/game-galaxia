...
import { rutaApp, rutaExterna, rutaMedios } from '../utils/rutas.js';
...
export class HandTracking {
  ...
  async activar() {
    ...
    try {
      ...
      const conjunto = await FilesetResolver.forVisionTasks(rutaExterna('mediapipe/wasm'));

      this.detector = await HandLandmarker.createFromOptions(conjunto, {
        baseOptions: {
          modelAssetPath: rutaMedios('models/hand_landmarker.task'),
          delegate: 'GPU',
        },
        runningMode: 'VIDEO',
        numHands: 2,
  ...
  async _cargarRostro() {
    ...
    try {
      ...
      this.detectorRostro = await FaceLandmarker.createFromOptions(conjunto, {
        baseOptions: {
          modelAssetPath: rutaMedios('models/face_landmarker.task'),
          delegate: 'GPU',
        },
        runningMode: 'VIDEO',
        numFaces: 1,

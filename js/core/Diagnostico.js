...
import { rutaApp, rutaVisible } from '../utils/rutas.js';
...
async function comprobarDatos() {
  const ruta = rutaApp('data/sistema-solar.json');
  const visible = rutaVisible('data/sistema-solar.json');
  try {
    const res = await fetch(ruta, { cache: 'no-cache' });
    if (!res.ok) {
      return {
...
async function comprobarBackend() {
  try {
    const res = await fetch(rutaApp('api/health.php'), { cache: 'no-store' });
